...
The task is framed the way a publisher would face it shortly before release.
Given what is knowable then -- platform, genre, rating, the studio's track
record, the franchise's history, and early critic reception -- rank a release
slate by the probability each title clears one million units. Because the
critic features only exist close to launch, this is a *pre-release
(near-launch)* forecast, not a greenlight-stage one; the ablation below
measures the model without them.
...
def write_report(rows: list[dict], leak: list[dict], ablation: list[dict],
                 ...
                 brier_before: float, brier_after: float, ci: dict, prov: dict) -> None:
...
**Decision point: {DECISION_POINT_LABEL}.** {DECISION_POINT_NOTE} The headline numbers below use the critic
features. The ablation section measures the model without them (PR-AUC {abl_strict:.3f} against {abl_full:.3f}), which is the
...
| | |
|---|---|
| Model version | {MODEL_VERSION} ({DECISION_POINT_LABEL.lower()}) |
| Code commit | `{(prov['code_commit'] or 'unknown')[:12]}`{' (code had uncommitted changes)' if prov['code_dirty'] else ''} |
| Cleaned data | SHA-256 `{prov['data_sha256']['clean_csv'][:16]}...`, {prov['data_rows_clean']:,} rows |
| Raw data | SHA-256 `{prov['data_sha256']['raw_csv'][:16]}...` |
| Training window / evaluation window | 1996-{TRAIN_END_YEAR} / {TRAIN_END_YEAR + 1}-{MODEL_MAX_YEAR} (temporal split) |
| Features | {len(prov['features'])} columns, schema hash `{prov['feature_schema_sha256'][:16]}...` |
| Libraries | Python {prov['python']}, scikit-learn {prov['scikit_learn']}, numpy {prov['numpy']}, pandas {prov['pandas']} |
...
def main() -> None:
    clean = load_clean()
    d = build_features(clean, train_end=TRAIN_END_YEAR)
    train, test = split_temporal(d, TRAIN_END_YEAR, MODEL_MAX_YEAR)
    prov = describe(FEATURES, data_files={"raw_csv": RAW_CSV, "clean_csv": CLEAN_CSV}, extra={
        "model_version": MODEL_VERSION, "decision_point": DECISION_POINT_LABEL,
        "training_window": f"1996-{TRAIN_END_YEAR}", "evaluation_window": f"{TRAIN_END_YEAR + 1}-{MODEL_MAX_YEAR}",
    ...
    write_report(rows, leak, ablation, imp, budget, len(train), len(test),
                 base_rate, best_name, brier_before, brier_after, ci, prov)
    ...
    (REPORTS_DIR / "metrics.json").write_text(json.dumps(
        {"models": rows, "ablation": ablation, "leakage_check": leak,
         ...
         "decision_point": DECISION_POINT_LABEL, "provenance": prov},
        indent=2,
    ))
